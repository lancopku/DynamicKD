...
from models.student import DynamicObjectiveKDForSequenceClassification
from models.callback import SupervisionWeightWriterCallback
...
class ModelArguments:
    ...
    kd_kl_alpha: Optional[float] = field(
        default=1.0, metadata={"help": "KD loss logit kl loss alpha"}
    )

    kd_rep_alpha: Optional[float] = field(
        default=1.0, metadata={"help": "KD loss rep loss alpha"}
    )
...
def main():
    ...
    trainer = Trainer(
        ...
        callbacks=[SupervisionWeightWriterCallback(model)]
    )
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            trainer.log_metrics("%s_eval" % task, metrics)
            trainer.save_metrics("%s_eval" % task, metrics)
